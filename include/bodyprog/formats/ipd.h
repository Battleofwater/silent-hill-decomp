...
STATIC_ASSERT_SIZEOF(s_IpdCollSurface, 12);
...
typedef struct _IpdCollSubcell
{
    ...
    /* 0x9    */ u8   surfaceIdx1;     // `s_IpdCollisionData::surfaces` index.
} s_IpdCollSubcell;
...
typedef struct _IpdCollisionData
{
    /* 0x0    */ q23_8                  positionX;
    /* 0x4    */ q23_8                  positionZ;
    /* 0x8+0  */ u32                    splitVertexCount : 8; /** `splitVertices` size. */
    /* 0x8+8  */ u32                    surfaceCount     : 8; /** `surfaces` size. */
